...
- **shadow/gridded_curved_DEM_NASADEM.py** Compute shortwave correction factor from NASADEM (geodetic coordinates, ~30 m resolution) for an example region in the Karakoram for a day in northern-hemisphere winter. Earth's surface curvature is considered and atmospheric refraction ignored. All non-glacier grid cells are masked to speed-up computation. An [Earthdata account](https://urs.earthdata.nasa.gov) is required and ['wget' has to be set](https://disc.gsfc.nasa.gov/data-access) to download NASADEM data.
