...
- **examples/gridded_curved_DEM.py**: Compute topographic parameters (slope angle/aspect, horizon and sky view factor) from SRTM (geodetic coordinates, ~90 m resolution) for a ~50 x ~50 km example region in the European Alps. Earth's surface curvature is considered. Plot output of this script is shown below.
...
- **examples/gridded_planar_DEM.py**: Compute topographic parameters (slope angle/aspect, horizon and sky view factor) from swisstopo DHM25 (map projection, 25 m resolution) for a ~25 x ~40 km example region in Switzerland. Earth's surface curvature is neglected.
