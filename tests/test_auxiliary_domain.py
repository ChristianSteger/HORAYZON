...
def test_rearrange_pad_buffer_preserves_vertices_and_aligns_buffer():
    ...
    expected = np.hstack((x.reshape(x.size, 1),
                          y.reshape(y.size, 1),
                          z.reshape(z.size, 1))).ravel()
...
def test_domain_helpers_reject_invalid_domains():
    with pytest.raises(ValueError):
        hray.domain.planar_grid({"x_min": 1.0, "x_max": 0.0,
                                 "y_min": 0.0, "y_max": 1.0})

    with pytest.raises(ValueError):
        hray.domain.curved_grid({"lon_min": 0.0, "lon_max": 1.0,
                                 "lat_min": 2.0, "lat_max": 1.0})
