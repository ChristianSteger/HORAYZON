...
def test_slope_methods_return_vertical_normals_for_flat_plane():
    ...
    for slope_func in (hray.topo_param.slope_plane_meth,
                       hray.topo_param.slope_vector_meth):
        vec_tilt = slope_func(x, y, z)
        np.testing.assert_allclose(vec_tilt[2, 2],
                                   [0.0, 0.0, 1.0],
                                   atol=1.0e-6)
...
def test_unobstructed_flat_sky_metrics_are_analytic():
    azim = np.linspace(0.0, 2.0 * np.pi, 8,
...
def test_constant_horizon_metrics_match_closed_form_values():
    ...
    azim = np.linspace(0.0, 2.0 * np.pi, 16,
                       endpoint=False, dtype=np.float32)
    ...
    np.testing.assert_allclose(svf, np.cos(horizon_angle) ** 2,
                               atol=1.0e-6)
    np.testing.assert_allclose(vsf, 1.0 - np.sin(horizon_angle),
                               atol=1.0e-6)
    np.testing.assert_allclose(top, np.pi / 2.0 - horizon_angle,
                               atol=1.0e-6)
