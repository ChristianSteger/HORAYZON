...
def undulation(lon_ip, lat_ip, geoid="EGM96"):
    ...
    if (
        sum((np.all(np.diff(lat_ip) > 0.0), np.all(np.diff(lat_ip) < 0.0)))
        != 1
    ):
        raise ValueError(
            "longitude values are not monotonically increasing ordecreasing"
        )
    ...
    if geoid == "EGM96":
        ...
        # Load data
        data = np.fromfile(
            path_aux_data + "EGM96/WW15MGH.GRD", sep=" ", dtype=np.float32
        )[6:]
        ...
        f_ip = interpolate.RectBivariateSpline(
            lat[::-1], lon_ra, np.flipud(data_ra), kx=1, ky=1
        )
    ...
    # -------------------------------------------------------------------------
        ...
        if not os.path.isdir(path_aux_data + "GEOID12A"):
            ...
            file_url = (
                "https://www.ngs.noaa.gov/PC_PROD/GEOID12A/"
        ...
        # Load data
        txt = (
            gzip.open(path_aux_data + "GEOID12A/g2012aa0.asc.gz", "r")
            .read()
            .decode("utf-8")
        )
        data = np.fromstring(
            "".join(txt.splitlines()), dtype=np.float32, sep=" "
        )[7:]
