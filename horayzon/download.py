...
def file(file_url, path_local):
    ...
    try:
        ...
        print(
            "SSL certificate verification failed - continue download "
            + "(yes/no)?"
        )
        cont = ""
        flag = False
...
def files(files_url, path_local, mode="parallel", block_size=500, file_num=10):
    ...
    if mode == "parallel":
        for i in range(0, len(files_url), block_size):
            ...
            downloaded.extend(
                executor.map(
                    get_file, files_url[i : i + block_size], repeat(path_local)
                )
            )
            executor.shutdown()
            files_count += len(files_url[i : i + block_size])
            print(
                "Files attempted to download: "
                + str(files_count)
                + "/"
                + str(len(files_url))
            )
