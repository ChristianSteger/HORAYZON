...
def planar_grid(domain, dist_search=50.0):
    ...
    # Check arguments
    if (domain["x_min"] >= domain["x_max"]) or (
    ...
    domain_outer = {
        "x_min": domain["x_min"] - (dist_search * 1000.0),
...
def curved_grid(domain, dist_search=50.0, ellps="sphere"):
    ...
    # Compute outer domain boundaries
    lat_abs_max = np.maximum(
        np.abs(domain["lat_min"]), np.abs(domain["lat_max"])
    )
    rad_sph = (
        a
        / np.sqrt(1.0 - e_2 * np.sin(np.deg2rad(lat_abs_max)) ** 2)
        * np.cos(np.deg2rad(lat_abs_max))
    )  # sphere radius [m]
    lon_add = 360.0 / (2.0 * np.pi * rad_sph) * (dist_search * 1000.0)  # [deg]
    domain_outer = {
        "lon_min": domain["lon_min"] - lon_add,
        "lon_max": domain["lon_max"] + lon_add,
        "lat_min": geod.Direct(
            domain["lat_min"], 0.0, 180.0, dist_search * 1000.0
        )["lat2"],
        "lat_max": geod.Direct(
            domain["lat_max"], 0.0, 0.0, dist_search * 1000.0
        )["lat2"],
    }

    # Check if total domain is within valid range (lon: -/+180.0, lat: -/+90.0)
    if (
        (domain_outer["lon_min"] < -180.0)
